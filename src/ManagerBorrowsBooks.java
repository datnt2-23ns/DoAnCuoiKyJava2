...
isEditable = true;
        ...
        bt_xoa.addActionListener(e -> {
            executorService.execute(() -> {
                try {
                    String maDocGia = JOptionPane.showInputDialog("Nhập mã độc giả cần xóa:").trim();
                    if (maDocGia != null) {
                        for (int i = 0; i < tableModel.getRowCount(); i++) {
                            if (tableModel.getValueAt(i, 1).equals(maDocGia)) {
                                tableModel.removeRow(i);
                    ...
                    JOptionPane.showMessageDialog(null, "Lỗi xảy ra khi xóa độc giả: " + ex.getMessage(), "Lỗi", JOptionPane.ERROR_MESSAGE);
                }
            });
        });
        ...
        bt_timKiem.addActionListener(e -> {
            executorService.execute(() -> {
                try {
                    String maDocGia = JOptionPane.showInputDialog("Nhập mã độc giả cần tìm:").trim();
                    if (maDocGia != null) {
                        for (int i = 0; i < tableModel.getRowCount(); i++) {
                            if (tableModel.getValueAt(i, 1).equals(maDocGia)) {
                                JOptionPane.showMessageDialog(null, "Độc giả có mã: " + maDocGia + " ở vị trí: " + (i + 1));
                    ...
                    JOptionPane.showMessageDialog(null, "Lỗi xảy ra khi tìm độc giả: " + ex.getMessage(), "Lỗi", JOptionPane.ERROR_MESSAGE);
                }
            });
        });
        ...
        bt_traSach.addActionListener(e -> {
            executorService.execute(() -> {
                try {
                    String maDocGia = JOptionPane.showInputDialog("Nhập mã độc giả trả sách:").trim();
                    if (maDocGia != null) {
                        for (int i = 0; i < tableModel.getRowCount(); i++) {
                            if (tableModel.getValueAt(i, 1).equals(maDocGia)) {
LocalDate currentDate = LocalDate.now();
                    ...
                    JOptionPane.showMessageDialog(null, "Lỗi xảy ra khi cập nhật trạng thái trả sách: " + ex.getMessage(), "Lỗi", JOptionPane.ERROR_MESSAGE);
                }
            });
        });
    }
